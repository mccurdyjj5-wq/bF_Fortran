...
```
bF2Fortran <input.b> [output.f90] [option]
```
The default output filename is 'output.f90'. It will replace the contents of the output destination if a file exists there already, so be careful! There is an option to not use the optimization feature. To do that, use the -u or --unoptimized command line options.
# Compatibility
